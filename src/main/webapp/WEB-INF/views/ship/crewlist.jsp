 ...
 <div class="cont-top">
    <div class="path">
      ...
      <div class="group depth2">
        <p class="label"><a href="/ship/position">직책정보</a></p>
        <ul>
...
<div id="sub_con" class="area">
	...
	<!-- 게시판 리스트 시작-->
		...
		<select size="1" name="search_txt">
			<option value="subject" >직책명</option>
			<option value="subject" >선종</option>
			<option value="subject" >선원명</option>
		</select>
	...
	<table style="width:100%;border:0; cellspacing:0; cellpadding:0;" summary=''>
      ...
      <tbody>
        ...
        <tr>
          <td scope="row" class="number">${crewlist.number}</td>
          <td class="title">${crewlist.vessel }</td>
          <td>${crewlist.position }</td>
          <td>${crewlist.name }</td>
		  <td>${crewlist.age }</td>
		  <td>${crewlist.nation }</td>
		  <td>${crewlist.boarding }</td>
		  <td>${crewlist.month }</td>
        </tr>
         </c:forEach>      
      </tbody>
